...
use synfx_dsp::crossfade;
use synfx_dsp::{AllPass, DCBlockFilter, DelayBuffer, OnePoleHPF, OnePoleLPF, TriSawLFO};
...
#[derive(Debug, Clone)]
pub struct DattorroReverbF32 {
    last_scale: f32,

    inp_dc_block: [DCBlockFilter<f32>; 2],
    out_dc_block: [DCBlockFilter<f32>; 2],

    lfos: [TriSawLFO<f32>; 4],

    input_hpf: OnePoleHPF<f32>,
    input_lpf: OnePoleLPF<f32>,

    pre_delay: DelayBuffer<f32>,
    input_apfs: [(AllPass<f32>, f32, f32); 4],

    apf1: [(AllPass<f32>, f32, f32); 2],
    hpf: [OnePoleHPF<f32>; 2],
    lpf: [OnePoleLPF<f32>; 2],
    apf2: [(AllPass<f32>, f32, f32); 2],
    delay1: [(DelayBuffer<f32>, f32); 2],
    delay2: [(DelayBuffer<f32>, f32); 2],
...
pub trait DattorroReverbParamsF32 {
...
impl DattorroReverbF32 {
    ...
    pub fn reset(&mut self) {
        ...
        self.input_apfs[0] = (
            AllPass::new(),
            DAT_INPUT_APF_TIMES_MS[0],
            DAT_INPUT_DIFFUSION1,
        );
        self.input_apfs[1] = (
            AllPass::new(),
            DAT_INPUT_APF_TIMES_MS[1],
            DAT_INPUT_DIFFUSION1,
        );
        self.input_apfs[2] = (
            AllPass::new(),
            DAT_INPUT_APF_TIMES_MS[2],
            DAT_INPUT_DIFFUSION2,
        );
        self.input_apfs[3] = (
            AllPass::new(),
            DAT_INPUT_APF_TIMES_MS[3],
            DAT_INPUT_DIFFUSION2,
        );
        ...
        self.apf1[1] = (
            AllPass::new(),
            DAT_RIGHT_APF1_TIME_MS,
            -DAT_PLATE_DIFFUSION1,
        );
        self.apf2[0] = (AllPass::new(), DAT_LEFT_APF2_TIME_MS, -DAT_PLATE_DIFFUSION2);
        self.apf2[1] = (
            AllPass::new(),
            DAT_RIGHT_APF2_TIME_MS,
            -DAT_PLATE_DIFFUSION2,
        );
    ...
    pub fn process(
        ...
        let out_pre_delay = if params.pre_delay_time_ms() < 0.1 {
            ...
            self.pre_delay
                .next_cubic(params.pre_delay_time_ms(), out_hpf)
        };
        ...
        let left = self.apf1[0]
            .0
            .next(left_apf1_delay_ms, self.apf1[0].2, left);
        ...
        let left = left * decay;
        let left = self.apf2[0]
            .0
            .next(left_apf2_delay_ms, self.apf2[0].2, left);
        ...
        let right = self.apf1[1]
            .0
            .next(right_apf1_delay_ms, self.apf1[1].2, right);
        ...
        let right = self.apf2[1]
            .0
            .next(right_apf2_delay_ms, self.apf2[1].2, right);
